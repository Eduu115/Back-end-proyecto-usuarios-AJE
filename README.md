...
1. **Crear la base de datos:**
   ```bash
   mysql -u root -p < src/main/resources/sql/create_db.sql
   ```

2. **Insertar datos iniciales:**
   ```bash
   mysql -u root -p < src/main/resources/sql/inserts.sql
   ```
...
```bash
curl -u usuario:contraseña http://localhost:8080/api/endpoint
```
...
⭐ Si este proyecto te ha sido útil, ¡no olvides darle una estrella!# Back-end Proyecto Usuarios AJE
